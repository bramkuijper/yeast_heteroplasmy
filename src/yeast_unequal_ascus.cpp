...
double ascus_intercept = 0;
double ascus_slope = 0;
...
void init_arguments(int argc, char *argv[])
{
    ...
    ascus_intercept = atof(argv[5]);
    ascus_slope = atof(argv[6]);
}
...
void make_tetrad(Individual *ascus, size_t ascus_size)
{
    ...
    for (size_t i = 0; i < ascus_size; ++i)
    {
        ...
        // to this tetrad
        size_t nmito_to_this_tetrad = nMito_min
        + (gsl_rng_uniform(r) < 0.5 ? -1.0 : 1.0) * gsl_rng_uniform_int(r, nMito_error);

        // set the begin count of mitochondria to 0
        // for each spore 
        ascus[i].nMito = nmito_to_this_tetrad;
        ascus[i].nMitoMut = 0;

        for (size_t j = 0; j < nmito_to_this_tetrad; ++j)
        {
            // sample a random mitochondrion
            ascus[i].nMitoMut += gsl_rng_uniform(r) < ascus_intercept + ascus_slope/3 * i;
        }
            
        assert(nmito_to_this_tetrad >= ascus[i].nMitoMut);
    }
}
...
void write_parameters()
{
    DataFile << endl << endl
                ...
                << "ascus_intercept;" << ascus_intercept << endl
                << "ascus_slope;" << ascus_slope << endl
                << "ch;" << ch << endl;
}
